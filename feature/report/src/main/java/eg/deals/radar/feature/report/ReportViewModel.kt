...
class ReportViewModel(
    ...
    fun submitReport() {
        ...
        // Reporting requires an account (prevents anonymous abuse)
        if (!eg.deals.radar.auth.AuthManager.isLoggedIn) {
            uiState = uiState.copy(error = AppLanguage.string(R.string.report_error_login))
            return
        }

        // Validation: Reason required
        if (uiState.selectedReason == null) {
            uiState = uiState.copy(error = AppLanguage.string(R.string.report_error_select_reason))
        ...
        if (isDetailsRequired() && !areDetailsValid()) {
            val reason = uiState.selectedReason!!.label(AppLanguage.isArabic())
            uiState = uiState.copy(
                error = AppLanguage.string(R.string.report_error_details, reason, MIN_DETAIL_LENGTH)
            )
        ...
        if (uiState.alreadyReported || deviceIdManager.hasReported(dealId)) {
            uiState = uiState.copy(error = AppLanguage.string(R.string.report_error_already))
        ...
        if (uiState.dailyLimitReached) {
            uiState = uiState.copy(error = AppLanguage.string(R.string.report_error_limit))
        ...
        viewModelScope.launch {
            try {
                ...
                if (reportData != null && reportData.isNotEmpty()) {
                    ...
                    val message = eg.deals.radar.network.ApiErrors.message(result, eg.deals.radar.network.ApiErrors.Context.REPORT)
                    uiState = when (result.code) {
                        eg.deals.radar.network.ApiErrors.ALREADY_DONE -> {
                            deviceIdManager.recordReport(dealId)
                            uiState.copy(loading = false, alreadyReported = true)
                        }
                        eg.deals.radar.network.ApiErrors.RATE_LIMITED -> uiState.copy(loading = false, dailyLimitReached = true)
                        else -> uiState.copy(loading = false, error = message)
                    }
                }
            } catch (e: Exception) {
                ...
                if (isDuplicate) {
                    ...
                    uiState = uiState.copy(
                        ...
                        error = AppLanguage.string(R.string.report_error_already)
                    )
                    return@launch
                }
                ...
                uiState = uiState.copy(
                    loading = false,
                    error = AppLanguage.string(R.string.report_error_network)
                )
            }
        }
    }}