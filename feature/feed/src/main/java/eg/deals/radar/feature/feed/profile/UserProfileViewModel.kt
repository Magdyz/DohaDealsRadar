...
import eg.deals.radar.db.UserEntity
            ...
            try {
                ...
                if (cachedUser != null) {
                    ...
                    if (result.isSuccess) {
                        ...
                        val error = result.exceptionOrNull()?.message ?: AppLanguage.string(R.string.account_error_load_profile)
                ...
                _uiState.update { it.copy(loading = false, error = AppLanguage.string(R.string.common_error_with_message, e.message ?: "")) }
            }
        }
    }
        ...
        viewModelScope.launch {
            try {
                ...
                if (result.isSuccess) {
                    ...
                    val error = result.exceptionOrNull()?.message ?: AppLanguage.string(R.string.profile_error_load_deals)
                ...
                _uiState.update { it.copy(dealsLoading = false, dealsError = AppLanguage.string(R.string.common_error_with_message, e.message ?: "")) }
            }
        }
    }
    ...
    val roleDisplayName: StateFlow<String> = _uiState
        .map { state ->
            when (state.user?.role) {
                "admin" -> AppLanguage.string(R.string.profile_role_admin)
                "moderator" -> AppLanguage.string(R.string.profile_role_moderator)
                "user" -> if (state.user.autoApprove) AppLanguage.string(R.string.profile_role_trusted) else AppLanguage.string(R.string.profile_role_user)
                else -> AppLanguage.string(R.string.profile_role_user)
            }
        }
        .stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5000),
            initialValue = AppLanguage.string(R.string.profile_role_user)
        )
