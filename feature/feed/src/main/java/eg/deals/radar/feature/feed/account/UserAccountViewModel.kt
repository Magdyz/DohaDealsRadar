...
import eg.deals.radar.network.UserDto
...
class UserAccountViewModel(
            ...
            try {
                ...
                if (userId != null) {
                    ...
                    _uiState.update { it.copy(
                        ...
                        error = AppLanguage.string(R.string.account_error_not_logged_in)
                ...
                _uiState.update { it.copy(error = AppLanguage.string(R.string.account_error_loading_user, e.message ?: "")) }
            ...
            try {
                ...
                if (result.isSuccess) {
                    ...
                    _uiState.update { it.copy(
                        ...
                        error = AppLanguage.string(R.string.account_error_load_profile)
                ...
                _uiState.update { it.copy(
                    ...
                    error = AppLanguage.string(R.string.common_error_with_message, e.message ?: "")
    ...
    fun logout() {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                eg.deals.radar.auth.AuthManager.logout(context)
    ...
    /** Fetches everything we store about the user as JSON text. */
    fun exportMyData(onResult: (Result<String>) -> Unit) {
        viewModelScope.launch { onResult(dealRepo.exportMyData()) }
    }

    /** Permanently deletes the account; logs out locally on success. */
    fun deleteAccount(onResult: (success: Boolean, message: String?) -> Unit) {
        viewModelScope.launch {
            val res = dealRepo.deleteAccount()
            if (res.success == true) {
                eg.deals.radar.auth.AuthManager.logout(context)
                _userId.value = null
                _uiState.value = UserAccountUiState()
                ...
                onResult(false, eg.deals.radar.network.ApiErrors.message(res))
            }
        }
    }
