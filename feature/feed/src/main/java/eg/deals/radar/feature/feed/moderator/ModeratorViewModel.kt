...
import eg.deals.radar.network.ReportWithDetailsDto
    ...
    private fun loadUserRole(userId: String) {
        viewModelScope.launch {
            try {
                ...
                if (cachedRole != null) updateRole(cachedRole)

                // Always refresh from the server so role changes apply without re-login
                run {
