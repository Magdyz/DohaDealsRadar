...
import eg.deals.radar.auth.AuthEvent
...
import eg.deals.radar.navigation.AppNavHost
import eg.deals.radar.design.theme.EgyptDealsTheme
// ✅ 1. ADD THIS IMPORT
...
class MainActivity : ComponentActivity() {
    ...
    // 🌐 Apply the in-app language (EN default / AR) before any resources are read
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 🔐 Session ended (refresh token revoked/expired): tell the user clearly
            ...
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                SessionStore.events.collect { event ->
                    if (event is AuthEvent.SessionExpired) {
                        Toast.makeText(
                            this@MainActivity,
                            AppLanguage.string(eg.deals.radar.core.data.R.string.session_expired_message),
                            Toast.LENGTH_LONG
                        ).show()
                    }
                }
